...
{if(typeof start=='undefined')start=0;var modal=_open_image_into_lightbox(imgs[start][0],imgs[start][1],start+1,imgs.length,true,imgs[start][2]);modal.positionInSet=start;var previous_button=document.createElement('img');previous_button.className='previous_button';previous_button.src='https://www.coeprobotics.com/portalbeta/themes/RSC/images/mediaset_previous.png'.replace(/^https?:/,window.location.protocol);var previous=function(e){if(typeof e=='undefined')e=window.event;cancel_bubbling(e);var new_position=modal.positionInSet-1;if(new_position<0)new_position=imgs.length-1;modal.positionInSet=new_position;_open_different_image_into_lightbox(modal,new_position,imgs);return false;};previous_button.onclick=previous;modal.left=previous;modal.box_wrapper.childNodes[0].appendChild(previous_button);var next_button=document.createElement('img');next_button.className='next_button';next_button.src='https://www.coeprobotics.com/portalbeta/themes/RSC/images/mediaset_next.png'.replace(/^https?:/,window.location.protocol);var next=function(e){if(typeof e=='undefined')e=window.event;cancel_bubbling(e);var new_position=modal.positionInSet+1;if(new_position>=imgs.length)new_position=0;modal.positionInSet=new_position;_open_different_image_into_lightbox(modal,new_position,imgs);return false;};next_button.onclick=next;modal.right=next;modal.box_wrapper.childNodes[0].appendChild(next_button);}
...
{if(typeof has_full_button=='undefined')has_full_button=false;var lightbox_code=' \
   <div style="text-align: center"> \
    <p class="ajax_loading" id="lightbox_image"><img src="'+'https://www.coeprobotics.com/portalbeta/themes/default/images/loading.gif'.replace(/^https?:/,window.location.protocol)+'" /></p> \
...
{return{WINDOW_SIDE_GAP:5,WINDOW_TOP_GAP:25,BOX_EAST_PERIPHERARY:4,BOX_WEST_PERIPHERARY:4,BOX_NORTH_PERIPHERARY:4,BOX_SOUTH_PERIPHERARY:4,VCENTRE_FRACTION_SHIFT:0.5,LOADING_SCREEN_HEIGHT:100,box_wrapper:null,button_container:null,returnValue:null,top_window:null,iframe_restyle_timer:null,open:function(){var options=arguments[0]||{};var defaults={'type':'alert','opacity':'0.5','width':'auto','height':'auto','title':'','text':'','yes_button':'Yes','no_button':'No','cancel_button':'Cancel','yes':null,'no':null,'finished':null,'cancel':null,'href':null,'scrollbars':null,'defaultValue':null,'target':'_self','input_type':'text'};this.top_window=window.top;this.top_window=this.top_window.top;for(var key in defaults){this[key]=(typeof options[key]!='undefined')?options[key]:defaults[key];}
...
if(_box_pos_top<this.WINDOW_TOP_GAP)_box_pos_top=this.WINDOW_TOP_GAP;_box_pos_left=((dim.window_width/2)-(parseInt(box_width)/2));box_pos_top=_box_pos_top+'px';box_pos_left=_box_pos_left+'px';this.box_wrapper.childNodes[0].style.top=box_pos_top;this.box_wrapper.childNodes[0].style.left=box_pos_left;var do_scroll=false;if(('1'==1)||(detected_box_height>dim.window_height)||(this.box_wrapper.style.position=='absolute'))
{var was_fixed=(this.box_wrapper.style.position=='fixed');this.box_wrapper.style.position='absolute';this.box_wrapper.style.height=((dim.page_height>(detected_box_height+bottom_gap+_box_pos_left))?dim.page_height:(detected_box_height+bottom_gap+_box_pos_left))+'px';this.top_window.document.body.style.overflow='';if((init)||(was_fixed))do_scroll=true;if((typeof iframe[0]!='undefined')&&(has_iframe_ownership(iframe[0]))&&(typeof iframe[0].contentWindow.scrolled_up_for=='undefined'))
{do_scroll=true;}}else
...
{_this.box_wrapper.childNodes[0].className=_this.box_wrapper.childNodes[0].className.replace(/ mousemove/g,'');}},2000);}};this.add_event(this.box_wrapper.childNodes[0],'click',function(e){try{_this.top_window.cancel_bubbling(e);}catch(e){}
if(_this.type=='lightbox')
{_this.option('right');}});switch(this.type)
...
{button=this.element('img',{'src':'https://www.coeprobotics.com/portalbeta/themes/default/images/button_lightbox_close.png'.replace(/^https?:/,window.location.protocol),'alt':this.cancel_button,'class':'overlay_close_button'});container.appendChild(button);}
